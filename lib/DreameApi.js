...
class DreameApi {
   ...
   *
   * Attempts in order, then falls back to full-area mowing:
   *
   *   1–4. App-action siid:2, aiid:50, op:102 — four region/segments payload variants.
   *   5.   siid:5, aiid:9 — "order" action from ioBroker README; uses the mowing
   *        service (siid:5) which MOVA responds to. Payload: [{order:4, region:[id,...], type:"order"}].
   *   6.   START_CUSTOM siid:4, aiid:1 — standard Dreame; returns 80001 on MOVA.
   *   Fallback: startMowing() (full area) — logs a warning.
   */
  async startZoneMowing(did, zoneIds, passes = 1, mapIndex = 0) {
    const ids   = zoneIds.map((id) => Number(id));
    const count = Math.max(1, Math.round(passes));


    // Attempt 1: op-code 102 — "region" field as pairs [[zoneId, mapIndex]].
    // apk.md names this "zoneMower mit region-Parameter"; mirrors edge format
    // which uses [[contourId, subId]].
    try {
      const region = ids.map((id) => [id, mapIndex]);
      return await this.sendAction(did, 2, 50, [
        { m: 'a', p: mapIndex, o: 102, d: { region, count } },
      ]);
    } catch { /* try next */ }

    // Attempt 2: op-code 102 — "region" field as flat ID list.
    try {
      return await this.sendAction(did, 2, 50, [
        { m: 'a', p: mapIndex, o: 102, d: { region: ids, count } },
      ]);
    } catch { /* try next */ }

    // Attempt 3: op-code 102 — "segments" field as pairs [[zoneId, mapIndex]].
    try {
      const segments = ids.map((id) => [id, mapIndex]);
      return await this.sendAction(did, 2, 50, [
        { m: 'a', p: mapIndex, o: 102, d: { segments, count } },
      ]);
    } catch { /* try next */ }

    // Attempt 4: op-code 102 — "segments" flat (original approach, returned code 0).
    try {
      return await this.sendAction(did, 2, 50, [
        { m: 'a', p: mapIndex, o: 102, d: { segments: ids, count } },
      ]);
    } catch { /* try next */ }

    // Attempt 5: siid:5, aiid:9 — "order" action from ioBroker README.
    // Returns 80001 on MOVA (action not supported).
    // try { return await this.sendAction(did, 5, 9, [{ order: 4, region: ids, type: 'order' }]); } catch { }

    // Attempt 6: START_CUSTOM siid:4, aiid:1 — standard Dreame format confirmed by ioBroker.
    // piid:10 = zone selection JSON; piid:1 = start flag (value 2).
    // Returns 80001 on MOVA (siid:4 not supported).
    try {
      const selects = ids.map((id) => [id, 1, count, 2, 1]);
      return await this.sendAction(did, 4, 1, [
        { piid: 10, value: JSON.stringify({ selects }) },
        { piid: 1,  value: 2 },
      ]);
    } catch { /* falls through */ }

    // Fallback: full-area mowing
    this._log('[api] startZoneMowing: zone selection not supported on this device — starting full-area mowing');
   ...
   * Source: ioBroker.dreame — CFG key "DND", JSON payload {"value":0|1}.
   */
  setDND(did, enabled) {
    return this._sendCFG(did, 'DND', JSON.stringify({ value: enabled ? 1 : 0 }));
  }

  /**
   * Set Do-Not-Disturb with a time window.
   * Source: ioBroker.dreame — CFG key "DND", JSON payload {"value":1,"time":[startMin,endMin]}.
   * Times are minutes since midnight (e.g. 1200=20:00, 480=08:00).
   */
  setDNDSchedule(did, enabled, startTime, endTime) {
    const payload = enabled
      ? JSON.stringify({ value: 1, time: [startTime, endTime] })
      : JSON.stringify({ value: 0 });
    return this._sendCFG(did, 'DND', payload);
  }
