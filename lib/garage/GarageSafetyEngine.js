...
const MAINTENANCE_ADJUSTING_MIN_MS = 20000;
const MAINTENANCE_ADJUSTING_MAX_MS = 30000;
...
class GarageSafetyEngine {
  ...
  _maintenanceCloseProtected() {
    ...
    const activePhase = ['exiting', 'adjusting', 'transit'].includes(phase);
  ...
  async _updateManualMaintenanceJourneyPhase() {
    ...
    if (this._manualMaintenancePhase !== 'adjusting') return;
    ...
    // The A2 visibly adjusts after leaving the garage. Preserve that phase for at
    // least 20 seconds; afterwards the first proven movement starts the transit.
    // A 30-second ceiling prevents missing/late telemetry from leaving the tile
    // stuck on Justieren forever.
    if ((phaseAge >= MAINTENANCE_ADJUSTING_MIN_MS && this._manualMaintenanceMovementSeen)
        || phaseAge >= MAINTENANCE_ADJUSTING_MAX_MS
        || commandAge >= MAINTENANCE_ADJUSTING_MAX_MS) {
