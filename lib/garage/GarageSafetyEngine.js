...
class GarageSafetyEngine {
  ...
  getTileStatus(nativeStatus) {
    // A manual trip to the native maintenance point is its own logical state.
    // The mower firmware may report `mowing` while opcode 109/index 2 is active;
    // never expose that misleading status in Homey.
  ...
  async _confirmReleasedStart(source, token, retryFn = null) {
    ...
    this._startConfirmNoticeTimer = setTimeout(async () => {
      ...
      const noMovementProof = !this._startConfirmedByNativeOrMovement() && !this._physicalOutsideRecoveryEvidence();
      if (!this._startRetrySent && this._startRetryFn && this.isDoorOpenStable() && stillHomeLike && noMovementProof) {
