...
class GarageSafetyEngine {
  ...
  _hasFreshSafetyPosition(maxAgeMs = 5000, requireAfterGateMotion = false) {
    ...
    // Never trigger an emergency from a position that was already old before this
    // opening/closing movement started.
  ...
  _confirmMovingGateDanger(moving, inside) {
    ...
    // Count only independent live-position updates — the same cached value must
    // never manufacture an emergency confirmation across consecutive poll cycles.
    ...
    // Closing is immediately dangerous → 1 independent sample is enough.
    // Opening emergency requires 2 independent samples to prevent a single cloud
    // packet from reversing a gate that is legitimately opening.
    const requiredHits = moving === 'opening' ? 2 : 1;
  ...
  getTileStatus(nativeStatus) {
    ...
    // while opcode 109/index 2 is active; never expose that misleading status.
    if (this._manualMaintenanceReached) return 'garage_maintenance_reached';
    if (this._manualMaintenanceTransit) {
      if (this._manualMaintenanceRecalibration) return 'garage_maintenance_transit';
      // Keep the visible manual-maintenance journey aligned with the physical
      // phases. Before the verified lawn-side crossing the mower is still in
      // Ausfahrt/Justieren and must never be shown as mowing or reached.
      if (!this._lineToLawnConfirmed) {
        const elapsed = this._outbound ? Date.now() - Number(this._outbound.startedAt || Date.now()) : 0;
        if (!this.isDoorOpenStable()) return 'garage_opening';
        if (elapsed < 10000) return 'garage_exiting';
        return 'garage_adjusting';
      }
  ...
  async requestClose(reason, opts = {}) {
    // While the explicit Homey maintenance-point journey is active, outbound
    // line dwell must never close the gate. The gate may close only after the
    // native index-2 arrival has been confirmed and this flag is cleared.
    if (this._manualMaintenanceTransit && !opts.emergencyReverse) {
      this.log('close suppressed during manual maintenance transit', reason || 'close');
      return false;
    }
    ...
    if (this._outboundCloseProtected() && !opts.maintenanceArrivalVerified) {
    ...
    if (this._missionOutside && !safelyDockedHomeForClose && !opts.requireExitConfirmed && !opts.maintenanceArrivalVerified) {
    ...
    if (!safelyDockedHomeForClose && closeModeGuard.blocked && !opts.maintenanceArrivalVerified) {
  ...
  async ensureDoorOpen(reason, opts = {}) {
    ...
    // A configured and healthy/online sensor remains authoritative. If it does
    // not confirm an open gate within the sensor timeout, do not release the mower
    // by timer: keep it paused/blocked. Timer proof is used only when sensor mode
    // is disabled or the sensor is unavailable/offline (handled above).
    this.log('door open sensor timeout; online sensor did not confirm open - release blocked');
    await this._setCap('garage_sensor_available_status', 'timeout');
    await this.safetyWarning('sensor_timeout_open_confirmation_missing');
    return false;
  }
  ...
  async maintenanceRequested(source, fn) {
    ...
    if (explicitManualMaintenance) {
      this.log('manual maintenance transit started', 'native_index_2');
  ...
  async onStatus(status, prevStatus) {
    ...
    // line/door-close logic take over before the mower reaches index 2.
    if (this._manualMaintenanceTransit) {
      this._missionOutside = true;
      this._lastOutdoorActivityAt = Date.now();
  ...
  _watchMaintenanceReached(callback, onTimeout = null) {
    ...
    this._maintenanceWatch = setInterval(async () => {
      ...
      if (reached) {
        ...
        // A manual maintenance departure from confirmed home is a controlled
        // calibration of the native opcode 109 / index 2 target. Only this narrow
        // path may store the physically reached mower coordinate as the immutable
        // visual maintenance marker. Normal return heuristics can never overwrite it.
        if ((this._manualMaintenanceCalibration || this._manualMaintenanceRecalibration)
            && typeof this.device._storeVerifiedIndex2MaintenancePoint === 'function') {
          const existingPoint = typeof this.device._resolveStableMaintenancePoint === 'function'
            ? await this.device._resolveStableMaintenancePoint().catch(() => null)
            : null;
          if (!existingPoint) {
            const reachedPos = this.pos();
            await this.device._storeVerifiedIndex2MaintenancePoint(reachedPos, 'manual_home_index2').catch(() => null);
            this.log('verified native index 2 maintenance marker stored', reachedPos ? `${Math.round(reachedPos.x)},${Math.round(reachedPos.y)}` : 'no-position');
          } else {
            this.log('existing maintenance marker preserved', `${Math.round(existingPoint.x)},${Math.round(existingPoint.y)}`);
          }
        } else if (typeof this.device._resolveStableMaintenancePoint === 'function') {
          await this.device._resolveStableMaintenancePoint().catch(() => null);
        }
        this._manualMaintenanceCalibration = false;
        this._manualMaintenanceRecalibration = false;
  ...
  async isAtMaintenancePointHeuristic() {
    ...
    // Controlled manual calibration path: the command started from confirmed
    // home, the garage was opened first, and the mower must have travelled away
    // from the dock. We therefore identify the native index-2 arrival by fresh,
    // stopped and position-stable telemetry instead of trusting the currently
    // displayed map marker (which is exactly what is being repaired).
    if (this._manualMaintenanceCalibration || this._manualMaintenanceRecalibration) {
      const current = this.pos();
      const elapsedMs = Date.now() - Number(this._maintenanceRequestedAt || Date.now());
      if (!current) {
        this._maintenanceStableHits = 0;
        return false;
      }
      // The departure reference can be unavailable or use a different native
      // coordinate source while the mower is docked. Do not make arrival
      // recognition depend exclusively on that reference. A confirmed outbound
      // safety-line crossing or a fresh position outside the danger area is also
      // hard proof that the mower has physically left the garage.
      const lineExitProven = !!this._lineToLawnConfirmed;
      const crossingPos = this._lastSafetyLineCrossing?.direction === 'garage->lawn'
        ? this._lastSafetyLineCrossing.position
        : null;
      const distanceFromCrossing = crossingPos ? this._distance(current, crossingPos) : null;
      const movedBeyondCrossing = Number.isFinite(Number(distanceFromCrossing))
        && Number(distanceFromCrossing) >= 500;

      // From home we retain the strict crossing + progress proof, preventing a
      // false marker while the mower is still adjusting in front of the garage.
      // When the explicit command starts outside, the user may already be on the
      // real native maintenance point. In that case no new line crossing exists;
      // the accepted native command plus a longer, stable stationary position is
      // the calibration proof.
      const departureProven = this._manualMaintenanceRecalibration
        ? elapsedMs >= 12000
        : (lineExitProven && movedBeyondCrossing);
      if (!departureProven) {
        this._maintenanceStableHits = 0;
        this._manualMaintenanceStableSince = 0;
      ...
      const statusAllowed = !['remote_control', 'mapping', 'error', 'returning'].includes(s);
      const stationary = drift != null && drift <= 250;
      const minimumTravelTimeReached = elapsedMs >= (this._manualMaintenanceRecalibration ? 12000 : 10000);

      if (!telemetryFresh || !statusAllowed || !stationary || !minimumTravelTimeReached) {
        this._maintenanceStableHits = 0;
        this._manualMaintenanceStableSince = 0;
        return false;
      }
      ...
      const requiredHits = this._manualMaintenanceRecalibration ? 4 : 3;
      const requiredStableMs = this._manualMaintenanceRecalibration ? 6000 : 4000;
      return this._maintenanceStableHits >= requiredHits && stableForMs >= requiredStableMs;
    }
