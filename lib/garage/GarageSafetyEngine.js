...
class GarageSafetyEngine {
  ...
  _confirmMovingGateDanger(moving, inside) {
    ...
    // A fresh live position inside the danger area while the gate is physically
    // opening or closing is already sufficient for the hard interlock. Keep the
    // existing state tracking so stale/invalid positions are rejected and the
    // emergency handler remains de-duplicated, but do not wait for a second poll.
    return this._gateMotionDangerHits >= 1;
  }
