...
class MovaApi {
        ...
        (res) => {
          ...
          stream.on('end', () => {
            ...
            try {
              ...
              //     quotes unescaped. We extract the raw {…} block before stripping
              //     because it may contain fault codes and other useful device fields.
              //
              ...
              try {
                const sanitized = raw
                  ...
                  // The property value is a JSON object embedded as an unescaped string.
                  // Inline it as a real JSON object so callers can read sub-fields
                  // (e.g. latestFaultCode). Falls back to null if the content is not
                  // parseable (nested braces, escape issues, etc.).
                  .replace(/"property"\s*:\s*"\{([^}]*)\}"(,?)/g, (_, inner, comma) => {
                    try {
                      JSON.parse(`{${inner}}`);
                      return `"property": {${inner}}${comma}`;
                    } catch {
                      return `"property": null${comma}`;
                    }
                  })
                  .replace(/(:\s*)(,|}|])/g, '$1null$2');
  ...
  async getDevices() {
    ...
    const list = (d.page && d.page.records)
        ...
        || (Array.isArray(d) ? d : []);
    // When the initial JSON parse succeeded (no malformed JSON), device.property is
    // a properly-escaped JSON string. Parse it into an object so device.js can read
    // sub-fields (e.g. latestFaultCode) without special-casing the string form.
