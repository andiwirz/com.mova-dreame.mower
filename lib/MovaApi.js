...
class MovaApi {
        ...
        (res) => {
          ...
          stream.on('end', () => {
            ...
            try {
              ...
              // Some MOVA API responses (newer devices, e.g. ViAX 300) return null-like
              // fields as bare `: ,` or `: }` — invalid JSON. Sanitize and retry.
              try {
                const sanitized = raw.replace(/(:\s*)(,|}|])/g, '$1null$2');
  ...
  /**
   * Read the PRE config array directly.
   * Some devices do not include PRE in the CFG response — this fetches it separately.
   * Returns the 19-element array, or null if not supported.
   */
  async getPRE(did) {
    const data = await this.sendAction(did, 2, 50, [{ m: 'g', t: 'PRE' }]);
    const out0 = data?.result?.out?.[0];
    const d    = out0?.d;
    // Device may return d as a bare array OR as a CFG-style object { PRE: [...] }.
    if (Array.isArray(d)) return d;
    if (Array.isArray(d?.PRE)) return d.PRE;
    return null;
  }
  ...
  /**
   * Set the blade cutting height in mm via the PRE config array (index 4).
   * Confirmed via packet capture: PRE[4] = 40 (mm).
   * Requires read-modify-write: caller must supply the current 19-element PRE array
   * from getCFG() so all other settings are preserved.
   * @param {number}   heightMm  Target cutting height in mm
   * @param {number[]} preArray  Current 19-element PRE array (from getCFG().PRE)
   */
  setCuttingHeight(did, heightMm, preArray = new Array(19).fill(0)) {
    const arr = [...preArray];
    arr[4] = Math.round(heightMm);
    return this._sendPRE(did, arr);
  }
  ...
  /**
   * @private Send a PRE config array.
   * PRE uses a different wire format from all other CFG keys:
   *   d = [...array...] directly — NOT d = { value: [...] }
   * Confirmed via packet capture (19-element array).
   */
  _sendPRE(did, preArray) {
    return this.sendAction(did, 2, 50, [{ m: 's', t: 'PRE', d: preArray }]);
  }
  ...
  // ─── PRE settings (siid:2, aiid:50) ─────────────────────────────────────
  //
  // PRE is a 19-element array sent as `d` directly (not wrapped in {value:...}).
  // Confirmed via packet capture:
  //   [0, 0, 0, 1, 40, 1, 7, 1, 1, 1, 1, 1, 1, 15, 15, 7, 1, 1, 5]
  //   Index 4 → cutting height (mm) ← confirmed 40 mm
  //   Index 1 → mow mode (0=Standard, 1=Efficient) ← from ioBroker source
  // Always requires read-modify-write: caller must supply the cached array from getCFG().

  /** Write a full PRE config array back to the device. */
  setPREConfig(did, preArray) {
    return this._sendPRE(did, preArray);
  }

  /**
   * Set mow efficiency mode (index 1 of the PRE array).
   * Caller must supply the current 19-element PRE array from getCFG() so all other
   * settings (cutting height etc.) are preserved.
   * @param {number}   value     0 = Standard, 1 = Efficient
   * @param {number[]} preArray  Current PRE array (19 elements)
   */
  setEfficientMode(did, value, preArray = new Array(19).fill(0)) {
    const arr = [...preArray];
    arr[1] = value;
    return this._sendPRE(did, arr);
  }
