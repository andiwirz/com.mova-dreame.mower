...
class MovaApi {
  ...
  /**
   * Anti-theft lift alarm (ATA): array [liftAlarmEnabled, 0, 0].
   * When enabled, the mower locks itself and triggers an alarm immediately when lifted.
   * Confirmed via packet capture — payload: { m:'s', t:'ATA', d:{ value:[0|1, 0, 0] } }
   * GET returns the same 3-element array; only index 0 is currently understood.
   * The trailing two zeros are preserved as-is to avoid disturbing unknown values.
   */
  setAntiTheftAlarm(did, enabled) {
    return this.sendAction(did, 2, 50, [{
      m: 's', t: 'ATA',
      d: { value: [enabled ? 1 : 0, 0, 0] },
    }]);
  }
